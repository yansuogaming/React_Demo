...
use Vietiso\Core\Http\File\File;
    ...
    public function sendContent(SwooleResponse $swooleResponse): static
    {
        try {
            ...
            while ($length && !$file->eof()) {
                ...
                if (false === $chunk = $file->fread($read)) {
                    break;
                }

                $read = $swooleResponse->write($chunk);
                if (false === $read || !$server->exist(App::service('request')->fd)) {
                    break;
                }
            }
            $swooleResponse->end();
            ...
            if (
                null === $this->tempFileObject
                && $this->deleteFileAfterSend
                && is_file($this->file->getPathname())
            ) {
