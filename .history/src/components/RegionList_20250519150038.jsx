    ...
    return (
        <section {...props} className={cn("container", className)}>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-[35px]">
                ...
                {regions.map((region, index) => (
                    <h2 key={index}  className="text-[#1A2A44] text-[40px] md:text-[56px] lg:text-[72px] font-bold">
                        {region.title}
                    </h2>
                    <p key={index}  className="text-[#494951] text-[16px] md:text-[18px] lg:text-[20px] font-normal mb-[30px] md:mb-[40px] lg:mb-[50px]">
                        {region.description}
                    </p>
