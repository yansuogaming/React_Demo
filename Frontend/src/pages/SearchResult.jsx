...
export default function SearchResults() {
  ...
  // Xử lý khi người dùng nhập vào input search
  const handleSearch = () => {
      ...
      <main className="max-w-7xl mx-auto px-4 py-24">
        ...
        <div className="relative mb-8">
          <div className="flex items-center bg-white rounded-md border border-gray-700">
            <Search 
              className="ml-3 h-5 w-5 text-black cursor-pointer" 
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          <div className="ml-auto">
            <div className="bg-white text-black text-lg px-3 py-1.5 rounded-full flex items-center transition-colors">
              10 results
            </div>
          </div>
        </div>
        ...
        {/* Search Results */}
        <div className="w-[100vw] relative left-1/2 translate-x-[-50%]">
          {/* Result 1 */}
          {Array.from({ length: 10 }).map((_, index) => (
            <div
              key={index}
              className="flex hover:bg-blue-100 duration-300 group hover:cursor-pointer transform transition-transform hover:translate-x-10"
            >
              <div className="container duration-300 gap-4 py-6 border-t border-gray-400 flex flex-col md:flex-row">
                <div className="flex-1">
                  <h2 className="text-2xl font-semibold mb-2 group-hover:text-blue-700">
                    Passports and Jobs
                  </h2>
